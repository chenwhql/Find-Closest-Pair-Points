...
int get_median(int* A, int p, int r)
{
	...
	int* median_set = new int[five_member_group_num + 1];
	int cnt = 0;

	for (int i = 0; i < n / 5; i++)
	{
		//sort(A + p + i * 5, A + p + i * 5 + 5);  //排序
		insertion_sort(A, p + i * 5, p + i * 5 + 4);
		median_set[cnt++] = A[p + i * 5 + 2];
	}
	if (last_group_member_num != 0)
	{
		//sort(A + n - last_group_member_num, &A[r]);
		insertion_sort(A, r - last_group_member_num + 1, r);
		median_set[cnt++] = A[r - last_group_member_num + 1 + (last_group_member_num - 1) / 2];
	}
	...
	{
		return get_median(median_set, 0, cnt - 1);
	}
...
double find_closest_pair_of_points(vector<Point>& P, int p, int r)
{
	...
	else  //还剩不止3个点
	{
		...
		//获取划分点集的轴
		int x_median = select(point_x, 0, cnt - 1, (r - p) / 2);
		//用完了释放
		if (point_x != NULL)
		{
			delete[] point_x;
