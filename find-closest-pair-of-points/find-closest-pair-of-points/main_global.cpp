...
vector<int> point_x;  //存放点x坐标，主要用来计算中位数
vector<Point> point_set_middle;  //存放轴两侧的点
vector<int> median_set_1; //存放各组的中位数,数组1
vector<int> median_set_2; //存放各组的中位数,数组2
int median_set_flag;  //选择使用数组1还是数组2
...
int get_median(vector<int>& A, int p, int r)
{
	...
	for (int i = 0; i < n / 5; i++)
	{
		...
		if (median_set_flag % 2 == 1)
		{
			median_set_1.push_back(A[p + i * 5 + 2]);
		}
		else
		{
			median_set_2.push_back(A[p + i * 5 + 2]);
		}
	}
	if (last_group_member_num != 0)
	{
		//sort(A.begin() + n - last_group_member_num, A.end());
		insertion_sort(A, r - last_group_member_num + 1, r);
		if (median_set_flag % 2 == 1)
		{
			median_set_1.push_back(A[r - last_group_member_num + 1 + (last_group_member_num - 1) / 2]);
		}
		else
		{
			median_set_2.push_back(A[r - last_group_member_num + 1 + (last_group_member_num - 1) / 2]);
		}
	}
	...
	if (median_set_flag % 2 == 1)
	{
		median_set_flag = (median_set_flag + 1) % 2;
		if (median_set_1.size() == 1)
		{
			return median_set_1[0];
		}
		else
		{
			return get_median(median_set_1, 0, median_set_1.size() - 1);
		}
	}
	else
	{
		median_set_flag = (median_set_flag + 1) % 2;
		if (median_set_2.size() == 1)
		{
			return median_set_2[0];
		}
		else
		{
			return get_median(median_set_2, 0, median_set_2.size() - 1);
		}
	}
